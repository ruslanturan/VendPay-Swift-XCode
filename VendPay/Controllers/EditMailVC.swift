...
class EditMailVC: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        txtMail.frame = CGRect(x: 30, y: logoHeight + 100, width: self.screenWidth - 60, height: 50)
        txtMail.delegate = self
        scrollView.addSubview(txtMail)
        btnConfirm.frame = CGRect(x: 20, y: logoHeight + 220, width: self.screenWidth - 40, height: 55)
    ...
    func changeMail(){
        ...
        DispatchQueue.main.async {
            [weak self] in
            self!.mailRequest(url: "https://www.vendpay.ge/user/editMail?id=" + ID[1] + "&mail=" + self!.txtMail.text!)
        }
    }
    func mailRequest(url: String){
        if let myURL = NSURL(string: url) {
            do {
                let myHTMLString = try NSString(contentsOf: myURL as URL, encoding: String.Encoding.utf8.rawValue)
                if (myHTMLString.lowercased.contains("error: e-mail")){
    ...
    func changeLanguage(str: String){
        txtMail.placeholder = "newMail".addLocalizableString(str: str)
        ...
        self.changed = "mailChanged".addLocalizableString(str: str)
